<table style="border-collapse: collapse;">
    <thead>
        <tr>
            <th colspan="8" style="background-color: #ffc000; color: #000000; text-align: center; font-weight: bold; border: 1px solid #000; height: 25px; vertical-align: middle;">
                {{ strtoupper($title) }}
            </th>
        </tr>
        <tr>
            ...
            <th style="font-weight: bold; border: 1px solid #000; text-align: center; background-color: #ffffff; color: #000000;">VA</th>
            <th style="font-weight: bold; border: 1px solid #000; text-align: center; background-color: #ffffff; color: #000000;">VA + TRANSFER</th>
    ...
    <tbody>
        @foreach($data as $row)
            <tr>
                ...
                <td style="border: 1px solid #000; text-align: right; background-color: #ffffff;">{{ \App\Services\MataUangFormatter::formatTotals($row['va_by_currency'] ?? []) }}</td>
                <td style="border: 1px solid #000; text-align: right; background-color: #ffffff;">{{ \App\Services\MataUangFormatter::formatTotals($row['va_transfer_by_currency'] ?? []) }}</td>
    ...
    <tfoot>
        <tr>
            ...
            <th style="border: 1px solid #000; text-align: right; font-weight: bold; background-color: #ffffff;">{{ \App\Services\MataUangFormatter::formatTotals($totals['va_by_currency'] ?? []) }}</th>
            <th style="border: 1px solid #000; text-align: right; font-weight: bold; background-color: #ffffff;">{{ \App\Services\MataUangFormatter::formatTotals($totals['va_transfer_by_currency'] ?? []) }}</th>
