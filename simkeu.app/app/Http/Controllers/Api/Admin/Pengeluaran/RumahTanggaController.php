...
class RumahTanggaController extends Controller
{
    ...
    public function exportBsi(Request $request)
    {
        ...
        return Excel::download(new \App\Exports\BsiPayrollExport($data, 'Pengeluaran Rumah Tangga'), 'CUZ BSI Pengeluaran Rumah Tangga.xlsx');
    }

    public function copyBsi(Request $request)
    {
        $data = $this->bsiRows($request);
        $export = new \App\Exports\BsiPayrollExport($data, 'Pengeluaran Rumah Tangga');
    ...
    protected function bsiRows(Request $request)
    {
        $query = KeuanganPengeluaranRumahTangga::query()
            ->select([
                'keuangan_pengeluaran_rumah_tangga.total as amount',
                'keuangan_pengeluaran_rumah_tangga.nama_kegiatan as message',
            ]);
        ...
        return $query
            ->where('keuangan_pengeluaran_rumah_tangga.jenis_pembayaran', 'CUZ BSI')
            ->orderBy('keuangan_pengeluaran_rumah_tangga.tanggal')
            ->get();
    }
